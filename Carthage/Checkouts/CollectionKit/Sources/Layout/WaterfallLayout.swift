...
public class CustomWaterfallLayout: VerticalSimpleLayout {
    ...
    public init(columns: Int = 2, interItemSpacing: CGFloat = 0, lineSpacing: CGFloat = 0) {
        self.columns = columns
    ...
    public override func simpleLayout(context: LayoutContext) -> [CGRect] {
      ...
      let columnWidth = (context.collectionSize.width - CGFloat(columns - 1) * interItemSpacing) / CGFloat(columns)
      var columnHeight = [CGFloat](repeating: 0, count: columns)
      ...
      for i in 0..<context.numberOfItems {
        ...
        cellSize = CGSize(width: columnWidth, height: cellSize.height)
        let (columnIndex, offsetY) = getMinColomn()
        columnHeight[columnIndex] += cellSize.height + lineSpacing
        let frame = CGRect(origin: CGPoint(x: CGFloat(columnIndex) * (columnWidth + interItemSpacing),
                                           y: offsetY),
                           size: cellSize)
        frames.append(frame)
      }
